...
export const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (
        ...
        await authState.keys.transaction(async () => {
            ...
            // Only for non-newsletter, non-group-status, non-media button messages
            ...
            if (
                ...
                hasButtonContent(message) &&
                !isMediaMsg
            ) {
                const bizNode = getBizNodeForButtons(message);
                if (bizNode) {
                    binaryNodeContent.push(bizNode);
                    logger.debug({ jid }, 'adding biz node for button message');
                }
            }
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            // Handles groupStatusMessage inline via relayMessage.
            // Using a separate class (GrpStatus) breaks status-seen and reactions
            // because it bypasses the standard relayMessage flow.
