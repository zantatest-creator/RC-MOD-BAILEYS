...
export const makeMessagesSocket = (config) => {
    ...
    const assertSessions = async (jids, force) => {
        ...
        const liveJids = uniqueJids.filter(j => !_deadDeviceJids.has(j));
        ...
        if (jidsRequiringFetch.length) {
            ...
            let batchOk = false;
            if (wireJids.length > 1) {
                try {
                    const result = await doPreKeyIQ(wireJids);
    ...
    const createParticipantNodes = async (recipientJids, message, extraAttrs, dsmMessage) => {
        ...
        const encryptionPromises = patchedMessages.map(async ({ recipientJid: jid, message: patchedMessage }) => {
            try {
                if (!jid || _deadDeviceJids.has(jid))
                    return null;
    ...
    // Helper to check if message has button content
    ...
    const getMessageType = (message) => {
        ...
        if (normalized?.reactionMessage || normalized?.encReactionMessage) {
            return 'reaction';
        }
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            // Check if this is a button message that needs a biz node
            const isButtonMessage = hasButtonContent(message);
            const isMediaMessage = !!getMediaType(message);
            
            // Only add biz node for standalone buttons (NO media)
            if (isButtonMessage && !isMediaMessage && !isNewsletter && !isRetryResend) {
